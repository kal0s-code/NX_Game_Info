...
{
    #pragma warning disable CA1422 // TODO: replace obsolete AppKit APIs (AllowedFileTypes, UsingColorSpace) when targeting modern macOS UI.
    public partial class MainWindowController : NSWindowController
    {
        private TableViewDataSource tableViewDataSource = null!;
        private TableViewDelegate tableViewDelegate = null!;
        ...
        public override void WindowDidLoad()
        {
            ...
            // TODO: migrate macOS settings to JSON-backed persistence to match other heads (Phase 4 follow-up).
            ...
            var copyMenu = contextMenu.ItemWithTitle("Copy")?.Submenu;
            if (copyMenu != null)
            {
                InitContextMenu(copyMenu);
            }
        ...
        public void OpenFile(NSMenuItem menuItem)
        {
            ...
            openPanel.BeginSheet(Window, (nint result) =>
            {
                ...
                {
                    var firstUrl = openPanel.Urls.FirstOrDefault();
                    if (firstUrl != null)
                    {
                        var initialDirectory = Path.GetDirectoryName(firstUrl.Path ?? string.Empty);
                        if (!string.IsNullOrEmpty(initialDirectory))
                        {
                            Common.Settings.Default.InitialDirectory = initialDirectory;
                            Common.Settings.Default.Save();
                        }
                    }

                    var filePaths = openPanel.Urls
                        .Select(arg => arg.Path)
        ...
        public void OpenDirectory(NSMenuItem menuItem)
        {
            ...
            openPanel.BeginSheet(Window, (nint result) =>
            {
                ...
                {
                    var firstUrl = openPanel.Urls.FirstOrDefault();
                    var selectedPath = firstUrl?.Path;
                    if (!string.IsNullOrEmpty(selectedPath))
                    {
                        Common.Settings.Default.InitialDirectory = selectedPath;
                        Common.Settings.Default.Save();
        ...
        public void OpenDirectory(string path)
        {
            ...
            if (!Directory.Exists(path))
            {
                new NSAlert()
                {
                    InformativeText = $"{path} is not a valid directory",
                    MessageText = NSBundle.MainBundle.ObjectForInfoDictionary("CFBundleExecutable").ToString(),
                }
                .RunModal();
                return;
        ...
        {
            if (Process.keyset is not KeySet keySet || keySet.SdCardEncryptionSeed.IsZeros())
            {
            ...
            if (keySet.SdCardKekSource.IsZeros() || sdCardKeySources.Length < 2 || sdCardKeySources[1].IsZeros())
            {
                Process.log?.WriteLine("Keyfile missing required keys");
                Process.log?.WriteLine(" - {0} ({1}exists)", "sd_card_kek_source", keySet.SdCardKekSource.IsZeros() ? "not " : "");
                Process.log?.WriteLine(" - {0} ({1}exists)", "sd_card_nca_key_source", (sdCardKeySources.Length < 2 || sdCardKeySources[1].IsZeros()) ? "not " : "");
            ...
            openPanel.BeginSheet(Window, (nint result) =>
            {
                if (result == (int)NSModalResponse.OK)
                {
                    ...
                    Common.Settings.Default.SDCardDirectory = sdPath;
                    Common.Settings.Default.Save();

                    title.StringValue = String.Format("Opening SD card on {0}", sdPath);
                    message.StringValue = "";
                    progress.DoubleValue = 0;
                    ...
                    backgroundWorker?.RunWorkerAsync((Worker.SDCard, sdPath));
                }
            });
        }
                    ...
                    if (filename.EndsWith(".csv", StringComparison.OrdinalIgnoreCase))
                    {
                        using (var writer = new StreamWriter(filename))
                        {
                            ...
                            string QuoteValue(string? value)
                                => (value ?? string.Empty).Quote(separator);
                            ...
                            foreach (var title in titles)
                            {
                                ...
                                writer.WriteLine(String.Join(separator.ToString(), new string[] {
                                    QuoteValue(title.titleID),
                                    QuoteValue(title.baseTitleID),
                                    QuoteValue(title.titleName),
                                    QuoteValue(title.displayVersion),
                                    QuoteValue(title.versionString),
                                    QuoteValue(title.latestVersionString),
                                    QuoteValue(title.systemUpdateString),
                                    QuoteValue(title.systemVersionString),
                                    QuoteValue(title.applicationVersionString),
                                    QuoteValue(title.masterkeyString),
                                    QuoteValue(title.titleKey),
                                    QuoteValue(title.publisher),
                                    QuoteValue(title.languagesString),
                                    QuoteValue(title.filename),
                                    QuoteValue(title.filesizeString),
                                    QuoteValue(title.typeString),
                                    QuoteValue(title.distribution.ToString()),
                                    QuoteValue(title.structureString),
                                    QuoteValue(title.signatureString),
                                    QuoteValue(title.permissionString),
                                    QuoteValue(title.error),
                                }));
                            }
                    ...
                    using (ExcelPackage excel = new ExcelPackage())
                        {
                            ...
                            worksheet.Cells[1, 1, 1, Title.Properties.Count].LoadFromArrays(new List<string[]> { Title.Properties.ToArray() });
                            ...
                            foreach (var title in titles)
                            {
                                ...
                                string lookupId = titleID ?? string.Empty;
                                Process.latestVersions.TryGetValue(lookupId, out uint latestVersion);
                                Process.versionList.TryGetValue(lookupId, out uint version);
                                Process.titleVersions.TryGetValue(lookupId, out uint titleVersion);
        ...
        public void UpdateTitleKeys(NSMenuItem menuItem)
        {
            ...
            int count = Process.Context?.Keyset.ExternalKeySet.ToList().Count ?? 0;

            if (Process.updateTitleKeys())
            {
                int updatedCount = (Process.Context?.Keyset.ExternalKeySet.ToList().Count ?? 0) - count;
                Process.log?.WriteLine("\nFound {0} updated title keys", updatedCount);

                Window.EndSheet(sheet);

                new NSAlert()
                {
                    InformativeText = String.Format("Found {0} updated title keys", updatedCount),
                    MessageText = NSBundle.MainBundle.ObjectForInfoDictionary("CFBundleExecutable").ToString(),
                }
                .RunModal();
            }
        ...
        public void UpdateVersionList(NSMenuItem menuItem)
        {
            ...
            if (Process.updateVersionList())
            {
                ...
                foreach (var title in titles)
                {
                    ...
                    {
                        if (Process.versionList.TryGetValue(title.baseTitleID ?? string.Empty, out uint version))
                        {
        ...
        {
            if (sender is not NSMenuItem menuItem)
            {
                return;
            }
            ...
            var pasteboardType = NSPasteboardType.String.GetConstant();
            pboard.DeclareTypes(new string[] { pasteboardType }, null);
        ...
        public void Rename(NSMenuItem menuItem)
        {
            ...
            tableView.SelectedRows.EnumerateIndexes((nuint idx, ref bool stop) =>
            {
                ...
                if (title != null)
                {
                    ...
                    string sanitizedTitle = Regex.Replace(title.titleName ?? string.Empty,
                        String.Format("[{0}]", Regex.Escape(new string(Path.GetInvalidFileNameChars()))), "");
                    string newNameCandidate = sanitizedTitle + " [" + title.titleID + "][v" + title.version + "]" + Path.GetExtension(filename);
                    string newname = directory != null
                        ? Path.Combine(directory, newNameCandidate)
                        : newNameCandidate;
        ...
        void BackgroundWorker_DoWork(object? sender, DoWorkEventArgs e)
        {
            ...
            else if (e.Argument is ValueTuple<Worker, string> argumentPath)
            {
                ...
                {
                    List<Title> sdTitles = Process.processSd(pathSd);
                    ...
                    foreach (var title in sdTitles)
                    {
                        ...
                        worker.ReportProgress(100 * index++ / Math.Max(1, count), title.filename);
                        titles.Add(title);
                    }
    ...
    {
        internal MainWindowController viewController = null!;
        ...
        public override bool AcceptDrop(NSTableView tableView, INSDraggingInfo info, nint row, NSTableViewDropOperation dropOperation)
        {
            ...
            {
                var pasteboardItems = pasteboard.PasteboardItems ?? Array.Empty<NSPasteboardItem>();
                string[] files = pasteboardItems
                    .Select(item => item.GetStringForType("public.file-url"))
                    .Where(urlString => !string.IsNullOrEmpty(urlString))
                    .Select(urlString => new NSUrl(urlString!).Path)
                    .Where(path => !string.IsNullOrEmpty(path))
                    .Select(path => path!)
                    .ToArray();
                ...
                {
                    viewController.OpenFile(existingFiles);
                    return true;
                }
            }
    ...
    public class TableViewDelegate : NSTableViewDelegate
    {
        ...
        public override NSView GetViewForItem(NSTableView tableView, NSTableColumn tableColumn, nint row)
        {
            ...
            string titleID = title.type == TitleType.AddOnContent
                ? (title.titleID ?? string.Empty)
                : (title.baseTitleID ?? string.Empty);
